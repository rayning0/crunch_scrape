...
CSV.open("/Users/user/sb/crunch_scrape/lib/cbase/nycompanies-2.csv", "wb") do |csv|
...
206237.upto(206629) do |i|
  begin
    puts "#{i}. #{company_permalinks[i]}"
    comp = JSON.parse(open("http://api.crunchbase.com/v/1/company/" + company_permalinks[i] + ".js?api_key=an35bya6x7ktkm5z7q3q6dvg").read)
  rescue StandardError => e
    $stderr.puts company_permalinks[i]
  ...
  comp["offices"].each do |office|
    CSV.open("/Users/user/sb/crunch_scrape/lib/cbase/nycompanies-2.csv", "ab") do |csv|
      if office["city"] == "New York"
        begin
          ...
          csv << [comp["name"], "=HYPERLINK(\"#{comp["homepage_url"]}\")", comp["phone_number"], 
            ...
            "=HYPERLINK(\"https://www.google.com/search?q='management+team'+#{company_permalinks[i]}\")", 
